...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            tail = True
        x = scheme_eval(expressions.first, env, tail)
        if expressions.rest is nil:
            return x
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('too {0} arguments to function call'.format('many' if len(formals) < len(vals) else 'few'))
        while formals is not nil:  
            cur_frame.define(formals.first, vals.first)
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail = False):
        ...
        "*** YOUR CODE HERE ***"
        if tail:
            return original_scheme_eval(result.expr, result.env, tail)
